...
uint8_t data_frame[264] = {0};											// буфер кадра ответа для измерения
bool response_ready = false;											// флаг готовности ответа (true - готов, false - не готов)
...
const uint32_t crc32_table[256] = { 									// рассчитанная таблица CRC-32
...
// формирует кадр ответа с заданным байтом, устанавливает флаг готовности ответа, запоминает сформированный ответ
void fillResponseFrame(uint16_t response);
...
bool checkCRC32(uint8_t* command_frame, uint16_t length) {
	// контрольная сумма полученной посылки
	uint32_t command_crc = (uint32_t)command_frame[9] |
							((uint32_t)command_frame[8] << 8) |
							((uint32_t)command_frame[7] << 16) |
							((uint32_t)command_frame[7] << 24);

	// расчет ожидаемой контрольной суммы для полученной посылки
	uint32_t calc_crc = calculateCRC32(command_frame, 5);
 ...
 * Возврат: результат проверки, представленный кодами ответов 0x07 или ID датчика */
